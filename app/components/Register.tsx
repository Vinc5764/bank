...
export default function Register() {
    ...
    const onSubmit = async (data: DonationFormData) => {
        try {
            ...
            const response = await axios.post('http://localhost:3001/api/register', mergedData);
    ...
    return (
        <div className="relative">
            <Card className="max-w-2xl mx-auto p-6 sm:p-8 md:p-10">
                <CardHeader>
                    <CardTitle className="text-3xl text-blue-950 font-bold">Register to Get Informed</CardTitle>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-1 gap-6">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                />
                                {errors.constituency && <p className="text-red-500">{errors.constituency.message as string}</p>}
                            </div>
            ...
            {isOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <p className="mb-4">Thank you for registering. You will receive updates soon.</p>
                        <Button onClick={closeModal} className="bg-blue-950 text-white px-4 py-2 rounded">Close</Button>
                    </div>
                </div>
            )}
        </div>
